class ProjectTagsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @project_tag.save
        format.html { redirect_to project_path(@project_tag.project_id), notice: 'Project tag was successfully created.' }
        format.json { render :show, status: :created, location: @project_tag }
      else
        format.html { redirect_to project_path(@project_tag.project_id) }
  ...
  private
    ...
    def project_tag_params
      params.require(:project_tag).permit(:tag_id, :project_id)
    end
end
